        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="h-9 px-3 rounded-full bg-lime-400/90 text-stone-900 flex items-center gap-1 active:scale-95 font-bold text-xs"
            >
              📡 SHARE
            </button>
            <button
...
function WeightsView({ weights, onSave, onBack }) {
  ...
  const renderPointRow = (group, [key, label, emoji]) => {
    ...
    return (
      <div key={key} className="flex items-center justify-between gap-3 py-2 border-b border-stone-100 last:border-b-0">
        ...
        <input
          ...
          className={`w-20 text-center font-display text-lg py-1 rounded-lg border-2 ${isNeg ? 'border-red-300 bg-red-50 text-red-700' : 'border-stone-200 bg-white text-stone-800'} focus:outline-none focus:border-stone-500`}
        />
      </div>
    );
  };
  ...
  const PointsSection = ({ group }) => (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border-2 border-lime-200 px-4 py-3">
        <div className="font-display text-sm text-lime-700 mb-1">ATTACKING</div>
        {ATK_ROWS.map(r => renderPointRow(group, r))}
      </div>
      <div className="bg-white rounded-2xl border-2 border-sky-200 px-4 py-3">
        <div className="font-display text-sm text-sky-700 mb-1">DEFENDING</div>
        {DEF_ROWS.map(r => renderPointRow(group, r))}
      </div>
      <div className="bg-white rounded-2xl border-2 border-amber-200 px-4 py-3">
  ...
  const PillarsSection = () => {
    ...
    const renderPillar = (role, label) => {
      ...
      const ok = s === 100;
      return (
        <div className="bg-white rounded-2xl border-2 border-stone-200 px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <div className="font-display text-sm text-stone-700">{label}</div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[['atk', 'ATK'], ['def', 'DEF'], ['dec', 'DEC'], ['inv', 'INV']].map(([k, lab]) => (
              <label key={k} className="flex flex-col items-center gap-1">
                <span className="text-[10px] font-display text-stone-500">{lab}</span>
                <input
                  type="number"
                  step="1"
                  min="0"
                  ...
                  className="w-full text-center font-display text-base py-1 rounded-lg border-2 border-stone-200 bg-white focus:outline-none focus:border-stone-500"
                />
              </label>
            ))}
          </div>
  ...
  const TabBtn = ({ id, label }) => (
    <button
      onClick={() => setTab(id)}
      className={`flex-1 py-2 font-display text-sm rounded-xl border-2 transition ${tab === id ? 'bg-stone-900 text-white border-stone-900' : 'bg-white text-stone-600 border-stone-200'}`}
    >
      {label}
    </button>
  );
  ...
  return (
    <div className="pb-32">
      <Header
        ...
        right={
          <button
            ...
            className="text-xs font-display text-stone-500 hover:text-stone-800 px-2 py-1 rounded-lg border border-stone-200"
          >
            RESET
          </button>
        }
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 px-4 py-3 flex gap-2">
        <button
          onClick={onBack}
          className="flex-1 py-3 rounded-xl border-2 border-stone-200 bg-white text-stone-700 font-display"
        >
          CANCEL
        </button>
        <button
          onClick={saveAndExit}
          className="flex-1 py-3 rounded-xl bg-lime-500 hover:bg-lime-600 text-stone-900 font-display border-2 border-lime-600"
        >
          SAVE
        </button>
      </div>
    ...
    <div className="pb-24 bg-stone-50 min-h-screen">
      ...
      <div className="px-4 pt-3 space-y-2.5">
        ...
        <Section id="live" emoji="📡" title="12 · Public scoreboard & coach access" summary="How URLs are split between parents and coaches">
          ...
          <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 mt-2 text-sm space-y-2">
            <div>
              <div className="font-display text-sm text-stone-700">👩‍👦 Parents — public scoreboard</div>
              <div className="text-xs text-stone-600">The bare URL (e.g. <code>stompers2016.com</code>) shows the live or most recent game scoreboard plus a list of all past matches. First name + jersey number only — no full names.</div>
            </div>
            <div>
              <div className="font-display text-sm text-stone-700">📎 Share a specific game</div>
              <div className="text-xs text-stone-600">Tap <Pill tone="lime">📡 SHARE</Pill> on any game to copy a link like <code>?live=GAME_ID</code> — great for tournament recaps or revisiting a past match in a TeamSnap event post.</div>
            </div>
            <div>
              <div className="font-display text-sm text-stone-700">🔑 Coaches — password-gated app</div>
              <div className="text-xs text-stone-600">Coach app lives at <code>?coach</code> (e.g. <code>stompers2016.com/?coach</code>). Bookmark this URL on your phone — enter the coach password once and it remembers your device.</div>
            </div>
          </div>
          <p className="text-xs text-stone-500 mt-2">Parents never need a password or to install anything — they just open the bare URL and bookmark it. Coaches install the PWA on the <code>?coach</code> URL for the full app experience.</p>
        </Section>
        ...
        <div className="px-4 pt-6 max-w-md mx-auto">
          ...
          <div className="bg-white border border-stone-200 rounded-2xl divide-y divide-stone-100 overflow-hidden">
            {past.map((g) => {
              ...
              const rColor = r === 'W' ? 'bg-lime-500 text-white' : r === 'L' ? 'bg-red-500 text-white' : 'bg-stone-300 text-stone-800';
              return (
                <a key={g.id} href={`./?live=${g.id}`} className="flex items-center gap-3 p-3 active:bg-stone-50">
  ...
  return (
    <>
      ...
      <div className="px-4 pt-5 max-w-md mx-auto">
        <h3 className="font-display text-xl text-stone-700 mb-2">GOALS</h3>
        {feed.length === 0 ? (
          <div className="bg-white border border-stone-200 rounded-2xl p-6 text-center text-sm text-stone-500">
            No goals yet.
          </div>
            ...
            {feed.map((row, idx) => {
              ...
              return (
                <React.Fragment key={idx}>
                  {isHt && (
                    <div className="bg-stone-100 px-3 py-1.5 text-center text-xs font-bold text-stone-500 uppercase tracking-widest">
                      — Half Time —
                    </div>
                  )}
                  {row.kind === 'us' ? (
                    <div className="p-3 flex items-start gap-3">
                      ...
                      <div className="text-2xl">⚽</div>
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-stone-900 truncate">{nameOf(row.scorerId)}</div>
                        {row.assistId && (
                          <div className="text-xs text-stone-500 truncate">🅰️ {nameOf(row.assistId)}</div>
                        )}
                      </div>
                    ...
                    <div className="p-3 flex items-start gap-3 bg-red-50/40">
                      ...
                      <div className="text-2xl">🚨</div>
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-stone-900 truncate">{game.opponent || 'Opponent'}</div>
                      </div>
